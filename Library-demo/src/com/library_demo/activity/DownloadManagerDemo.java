	...
	private Handler _handler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			if (status == DownloadManager.STATUS_RUNNING && max > 0) {
				...
				Toast.makeText(DownloadManagerDemo.this,
						"下载成功，文件保存路径:" + filePath, Toast.LENGTH_SHORT).show();
	...
	private void inital() {
		...
		_btn_download.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				...
				_downloadId = _downloadUtil.startDownload(
						HttpConstants.DownloadUrl,
						Uri.fromFile(new File(filePath)));

				observer = new DownloadStatusObserver(_handler);
				DownloadManagerDemo.this.getContentResolver()
						.registerContentObserver(
								LibConstant.DownloadInfoDataBase_ContentUri,
								true, observer);
				_receiver = new DownloadResultReceiver();
				DownloadManagerDemo.this.registerReceiver(_receiver,
						new IntentFilter(
	...
	private class DownloadResultReceiver extends BroadcastReceiver {
		...
		public void onReceive(Context context, Intent intent) {
			...
			if (intent.getAction().equals(
					DownloadManager.ACTION_DOWNLOAD_COMPLETE)) {
				HashMap<String, Object> one = _downloadUtil
						.getCurDownloadInfo(_downloadId);
				String filePath = (String) one
						.get(DownloadManager.COLUMN_LOCAL_FILENAME);
